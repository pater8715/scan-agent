...
|-------|-------------|---------------------|
| Target | IP, hostname, URL o rango CIDR | `juice-shop` / `172.20.0.0/24` |
...
Puedes detener un escaneo en cualquier momento de dos formas:

- **Desde el formulario principal:** aparece un botón **Cancelar** mientras el escaneo está activo.
- **Desde la barra de escaneos activos:** cada chip de escaneo en curso muestra un botón `⛔` para cancelarlo directamente.
- **Desde el historial:** los escaneos con estado `running` o `pending` muestran un botón `⛔ Cancelar`.

La cancelación es inmediata: termina el proceso nmap subyacente y el escaneo pasa a estado `cancelled`.

### Escaneo de red (CIDR) y hosts descubiertos

Puedes escanear rangos de red completos usando notación CIDR como target:
...
Al terminar un escaneo de red con el perfil `network`, aparece automáticamente un panel **"Hosts Descubiertos"** con:

- Tarjeta por cada host activo encontrado: IP, hostname, latencia y puertos abiertos
- Selector de perfil individual por host
- Botón **"Escanear"** para lanzar un escaneo profundo sobre ese host específico

Esto permite un flujo de dos fases: primero descubres qué equipos están activos en la red, luego escaneas en detalle los que te interesen.

**Ejemplo de flujo en el lab:**

1. Target: `172.20.0.0/24` | Perfil: `network` → descubre juice-shop, dvwa, zap
2. En el panel de hosts, selecciona el perfil `lab` para juice-shop → haz clic en **Escanear**
3. Se lanza automáticamente un nuevo escaneo profundo sobre esa IP
...
4. Si fue un escaneo de red, aparece además el panel de hosts descubiertos (ver arriba)
...
| `172.20.0.0/24` | Red completa del lab (descubrimiento CIDR) |
...
- `GET /api/scans/{scan_id}/hosts` — listar hosts activos descubiertos (escaneos CIDR)
- `DELETE /api/scans/{scan_id}` — cancelar un escaneo en curso

...
async def list_all_reports():
    """Lista todos los escaneos que tienen reportes disponibles."""
    reports_dir = Path("./reports")
    if not reports_dir.exists():
        return []

    scans: dict = {}
    for f in sorted(reports_dir.glob("scan_*.*"), key=lambda x: x.stat().st_mtime, reverse=True):
        if not f.is_file():
            continue
...
@router.get("/{scan_id}/view/html", response_class=HTMLResponse)
async def view_report_html(scan_id: str):
    """Sirve el reporte HTML para visualización inline (sin forzar descarga)."""
    report_path = Path(f"./reports/scan_{scan_id}.html")
    if not report_path.exists():
        raise HTTPException(status_code=404, detail="Reporte HTML no encontrado")
    return HTMLResponse(content=report_path.read_text(encoding="utf-8"))


@router.get("/{scan_id}/view/txt", response_class=PlainTextResponse)
async def view_report_txt(scan_id: str):
    """Sirve el reporte TXT para visualización inline."""
    report_path = Path(f"./reports/scan_{scan_id}.txt")
    if not report_path.exists():
        raise HTTPException(status_code=404, detail="Reporte TXT no encontrado")
